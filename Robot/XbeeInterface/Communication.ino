...
void checkForIns() {
  ...
  if (xbee.getResponse().isAvailable()) {
    ...
    switch (response.getApiId()) {
      ...
      // This *should* never come up, as the Arduino should wait for status response right after
      // sending a tx
...
void handleRx16() {
  ...
  // Get reference to frame data
  // Frame data includes
  uint8_t *data = rx16.getFrameData();
  // Get RSSI
  uint8_t rssi = rx16.getRssi();
  // Update payload to echo received data
  for (int i = 0; i < dataLength; i++) {
//    Serial.print(*(data+i+rx16.getDataOffset()), HEX);
//    Serial.print(' ');
    *(payload + i) = *(data + rx16.getDataOffset() + i);
  }
//  Serial.println();
  // Send RSSI as last byte
  *(payload + dataLength) = rssi;
